...
def path_finder(present_node):
    if present_node.parent_node() != None:
        return path_finder(
            present_node.parent_node()) + ' ' + present_node.node_name()
    else:
        return present_node.node_name()


def bfs_start(curr_node, dest_node, graph_data, visited_nodes,
              visited_node_names):
            ...
            if dest_node not in visited_node_names:
                ...
                for child in next_nodes:
                    if child[0] not in visited_node_names:
                        next_node = present_node.add_child(child[0],
                                                           int(child[1]))
...
def dfs_start(curr_node, dest_node, graph_data, visited_nodes,
              visited_node_names):
    src = Node(curr_node)
    pipe = []
    pipe.append(src)
...
def dls_start(curr_node, dest_node, graph_data, depth, visited_nodes,
              visited_node_names):
    result = ""
        ...
        while True:
            ...
            for child in next_nodes:
                if child[0] not in visited_node_names:
                    ...
                    result = dls_start(next_node, dest_node, graph_data,
                                       depth - 1, visited_nodes,
                                       visited_node_names)
                    if result == "success":
                        return result
            result = "cutoff"
...
def idls_start(curr_node, dest_loc, graph_data, depth):
    ...
    while state == "cutoff":
        visited_nodes = []
        visited_node_names = []
        state = dls_start(curr_node, dest_loc, graph_data, step,
                          visited_nodes, visited_node_names)
        if state == "cutoff":
            step = step + depth
        else:
            print(state)
...
def algo_in():
    algo = input(
        "Enter the desired Algorithm(valid inputs are: 'BFS','DFS','IDS')")
...
def uninfromed_search(file_name):
    ...
    if algo == "BFS":
        bfs_start(src_loc, dest_loc, graph_data, visited_nodes,
                  visted_node_names)
    elif algo == "DFS":
        dfs_start(src_loc, dest_loc, graph_data, visited_nodes,
                  visted_node_names)
    elif algo == "IDS":
        depth = depth_in()
