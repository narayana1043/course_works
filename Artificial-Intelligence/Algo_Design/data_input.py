...
class DataInput(object):
    ...
    def src_in(self, places):
        self.src_loc = "Arad"#input("Enter the source point from the list above")
        if self.src_loc not in places:
            self.src_in(places)
        return self.src_loc

    def dest_in(self, places):
        self.dest_loc = "Bucharest"#(input("Enter the destination point from the list above"))
        if self.dest_loc not in places:
            self.dest_in(places)
        return self.dest_loc

    def algo_in(self):
        self.algo = "BFS"#(input("Enter the desired Algorithm(valid inputs are: 'BFS','DFS')"))
    ...
    def __init__(self,file_name):
        ...
        sample_graph = Graph()
        self.graph_data, self.graph_weight = sample_graph.graph_gen(self.src_loc,self.dest_loc,self.dist_bw_places)
