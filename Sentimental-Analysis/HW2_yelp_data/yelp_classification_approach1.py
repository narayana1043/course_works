...
3. Metrics used: classification report, confusion matrix, accuracy score,
...
from sklearn.metrics import confusion_matrix as cm, accuracy_score as acc_score
from sklearn.metrics import precision_score as pre_score
from sklearn.metrics import recall_score as rc_score
...
# pruning the review using regular expressions inorder to split the review into
#  a list of words
def prune_words(line_words_preprocess):
    for word in line_words_preprocess:
        line_words_preprocess[line_words_preprocess.index(word)] = re.sub(
            r'([$\W])', '', word.lower())
...
def bag_of_words(file_name):
    ...
    while line:
        ...
        line_words = prune_words(list(set(review["text"].split())))
        ...
        if i >= samples:
            break
        line = f.readline()
    # increasing leng of all vector models to the length of final bag of words
