...
pos_filter = ['CC', 'CD', 'DT', 'EX', 'FW', 'IN', 'LS', 'MD', 'NN', 'NNS',
              'NP', 'NPS',
              'PDT', 'PP', 'PP$', 'SYM', 'TO', 'WDT', 'WP', 'WP$', 'WRB']
...
def prune_words(review_text):
    ...
    for sentence in review_sent_split:
        ...
        review_word_bag += [(stemmer.stem(tagged_word[0])) for tagged_word in
                            pos_tagged_words if (
                                tagged_word[0] not in stop_words and
                                tagged_word[
                                    1] not in pos_filter)]
...
def classifier(file_name):
    ...
    print("The fined grained Accuracy for this case is :   %d" % (
        (1 - error) * 100))
