...
2.Improving performance with conda or theano by utilizing GPU --- work in progress hoping to complete soon -- unable to install theano
    reason - on windows to install theano there are other mandatory stuff that needs to handled with care
...
#nltk.download('punkt')                                  #downloading for punkt is used for part of speech tagging
#nltk.download('stopwords')                              #downloading stopwrods from english language
...
def pruneWords(review_text):
    ...
    for sentence in review_sent_split:
        ...
        review_word_bag += [stemmer.stem(word) for word in words if word not in stop_words]         #removing stopwords and stemming
