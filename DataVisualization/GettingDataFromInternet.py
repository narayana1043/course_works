...
def graph_data(stock):
    ...
    ax1.plot_date([], [], linewidth=5, label='loss', color='r', alpha=0.5,
                  fmt='-')
    ax1.plot_date([], [], linewidth=5, label='gain', color='g', alpha=0.5,
                  fmt='-')
    ax1.axhline(close_price[0], color='k', linewidth=2)
    ax1.fill_between(date, close_price, 11,
                     where=(close_price > close_price[0]), facecolor='g',
                     alpha=0.3)
    ax1.fill_between(date, close_price, 11,
                     where=(close_price < close_price[0]), facecolor='r',
                     alpha=0.3)
    ...
    ax1.tick_params(axis='x',
                    colors='#f06215')
    # color code('#f06215') from color hex website
