...
def stopCondition(stop_list,class_column):
    temp_list = []
...
def validate(v_list):
    ...
    for v_item in v_list:
        v_predicted_list.append(classFinder(v_item,Node.root))
    for v_index in range(len(v_list)):
        if v_list[v_index][class_column] == v_predicted_list[v_index]:
            count += 1
    return ((count/len_v_list)*100)
...
def accuracy(test_list,predicted_list,class_column):
    ...
    for index in range(len_test_list):
        if test_list[index][class_column] == predicted_list[index]:
            count += 1
    return ((count/len_test_list)*100)
    ...
    for i in range(10):                                                                   #fold range
        ...
        while (mark < int(len(random_list))):
            for train_ele in range(0,mark):
                ...
                v_list = random.sample(list(training_list),int(len(training_list)*0.25))
                for v_item in v_list:
                ...
                for test_element in test_list:
                    predicted_list.append(classFinder(test_element,temp_root))
                acc_list.append(accuracy(test_list,predicted_list,class_column))
                break
    print(mean(acc_list))
        #print(test_list)
