...
def buildTree(node):
    ...
    if node.stop_condition == True:
        ...
        while Node.children != []:
            ...
            mdl = (Node.node_count - Node.leaf_node_count)*log2(len(column_names))+(Node.leaf_node_count*log2(Node.class_count))+(sum_node_err*log2(sum_node_err))
            if mdl < Node.old_mdl:
                Node.old_mdl = mdl
                Node.children = Node.new_children
                Node.new_children = []
...
def startSplitData(data_list):
    ...
    for i in range(10):                                                                   #fold range
        ...
        # fold completion
                ...
                Node.old_mdl = (Node.node_count - Node.leaf_node_count)*log2(len(column_names))+(Node.leaf_node_count*log2(Node.class_count))+(len(data_list)*log2(len(data_list)))
