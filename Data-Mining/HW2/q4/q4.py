...
def count_values_same_class(list, class_column, class_values):
    value_dict = {}
    count_value_dict = {}
...
def split_weight_find(count_value_dict, class_values):
    ...
    for value in class_values:
        if count_value_dict[value] != 0 and formula == 0:
            ...
            split_weight -= (count_value_dict[value] / sum_values) * log10(
                (count_value_dict[value] / sum_values) ** 2)
...
def stop_condition(list, class_column):
    temp_list = []
    for element in list:
        # print(element)
...
def build_tree(node):
    ...
    node.stop_condition = stop_condition(node.data, class_column)
    if node.stop_condition == True:
        ...
        # print(node.name)
        node.left_list, node.right_list = spliting_tree(node.data, split_col,
                                                        split_value)
...
for file_loc in range(9):
    ...
    if formula_input == formula_list.index('Information Gain'):
        ...
        print('The Accuracy measures on %s for 10-fold validation using '
              'Information Gain' % (files_names[file_loc][0]))
        ...
        for measure in formula_list:
            # formula = int(input('Enter 0 for Information Gain and '
            #                     '1 for Gene'))
