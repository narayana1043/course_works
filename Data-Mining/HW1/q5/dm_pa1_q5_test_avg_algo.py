...
def estimate(test_user, test_movie, user_movie_review,movie_user_review,predictor_type,i):
    ...
    for curr_user in movie_user_review[test_movie].keys():
        ...
        intersection_watch_list = set(test_user_watch_list).intersection(set(curr_user_watch_list))
        if len(intersection_watch_list) > 10:
            if predictor_type == "ED":
                rating_difference = rating_ed_diff_gen(intersection_watch_list, test_user, curr_user, movie_user_review)
            elif predictor_type == "MD":
                rating_difference = rating_md_diff_gen(intersection_watch_list, test_user, curr_user, movie_user_review)
            elif predictor_type == "Lmax":
                rating_difference = rating_lmax_diff_gen(intersection_watch_list, test_user, curr_user, movie_user_review)
            rating_difference_map.append([curr_user, rating_difference/len(intersection_watch_list)])
        else:
            rating_difference_map.append([curr_user, mean_find(user_movie_review,movie_user_review,test_movie)])
    sorted_rating_diff_map = sorted(rating_difference_map, key=itemgetter(1))[:i]        # k Value in question is in the square braces here
    #print(sorted_rating_diff_map)
    return top_pick_rating(sorted_rating_diff_map, test_movie, movie_user_review,i)
    #return mean_find(user_movie_review, movie_user_review, test_movie)

def ed_md_lmax(user_movie_review, movie_user_review, test_user_movie_review, predictor_type, i):
    predicted_user_movie_reviews = {}
    for user in list(test_user_movie_review.keys()):#[:1]:
        ...
        for movie in test_user_movie_review[user].keys():
            if movie in movie_user_review.keys():
                predicted_user_movie_reviews[user][movie] = estimate(user, movie, user_movie_review, movie_user_review,predictor_type,i)
...
def predictor(user_movie_review, movie_user_review, test_user_movie_review,predictor_type,i):
    if predictor_type == "AVG":
        result = avg(user_movie_review, movie_user_review, test_user_movie_review)
        return result
    else:
        result = ed_md_lmax(user_movie_review, movie_user_review, test_user_movie_review,predictor_type,i)
        return  result
...
for i in [5,7,10]:
    ...
    result = predictor(user_movie_review, movie_user_review, test_user_movie_review, "ED",i)
    print("Eculidean Distance   :",result)
    result = predictor(user_movie_review, movie_user_review, test_user_movie_review, "MD",i)
    print("Manhattan Distance   :",result)
    result = predictor(user_movie_review, movie_user_review, test_user_movie_review, "Lmax",i)
