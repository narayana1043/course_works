...
def estimate(test_user, test_movie, user_movie_review, movie_user_review):
    ...
    for curr_user in movie_user_review[test_movie].keys():
        # print(user)
        # curr_movie_watched_user_list = \
        #     [watched_user for movie in movie_user_review.keys()
        #                     if movie == curr_movie for watched_user in
        #                                 movie_user_review[movie].keys()]
        curr_user_watch_list = user_movie_review[curr_user].keys()
        intersection_watch_list = list(
            set(test_user_watch_list).intersection(set(curr_user_watch_list)))
        if len(intersection_watch_list) > 0:
            rating_difference = rating_diff_gen(intersection_watch_list,
                                                test_user, curr_user,
                                                movie_user_review)
...
def ed(user_movie_review, movie_user_review, test_user_movie_review,
       test_movie_user_review):
    predicted_user_movie_reviews = {}
    for user in test_user_movie_review.keys():
        predicted_user_movie_reviews[user] = {}
        ...
        for movie in test_user_movie_review[user].keys():
            ...
            # predicted_user_movie_reviews[user] = {
            #     movie: estimate(user, movie, user_movie_review,
            #                     movie_user_review)}
            if movie in movie_user_review.keys():
                predicted_user_movie_reviews[user].update({movie: estimate(
                    user, movie, user_movie_review, movie_user_review)})
                difference.append(abs(
                    int(test_user_movie_review[user][movie]) - int(
                        predicted_user_movie_reviews[user][movie])))
        result = mean(difference)
...
def predictor(user_movie_review, movie_user_review, test_user_movie_review,
              test_movie_user_review, predictor_type):
    result = 0
    if predictor_type == "AVG":
        ...
        result = ed(user_movie_review, movie_user_review,
                    test_user_movie_review, test_movie_user_review)
        return result

l = [["u1.base", "u1.test"], ["u2.base", "u2.test"], ["u3.base", "u3.test"],
     ["u4.base", "u4.test"], ["u5.base", "u5.test"]]
...
for i in l:
    ...
    result = predictor(user_movie_review, movie_user_review,
                       test_user_movie_review, test_movie_user_review,
                       "ED")  # input("Predictor Type: Values(AVG)"))
    print(result)
