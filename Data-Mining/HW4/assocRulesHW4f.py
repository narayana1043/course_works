...
#association rule generator to generate association rules on the frequent itemsets at each level based on lift
#Note rules cannot be pruned based on lift
def ruleGen(freqItemList):
    ruleGen = [[] for _ in range(len(freqItemList)-1)]
    #ruleGen1 = [[] for _ in range(len(freqItemList)-1)]
    ...
    for level in range(len(freqItemList)):
        ...
        for itemSet in freqItemList[level]:
            ...
            for combination in combinationsGenerated:
                ...
                ruleLift = (itemSet[1]/combinationCount)/(itemSet[1]/len(searchSpaceItemSets))      #lift = target response/avg response
                ruleGen[level].append([combination,implicationSet,ruleLift])
        #print(len(ruleGen[level]))
    return ruleGen

#top 10 rules Generator for a given set of rules
def top10RuleGen(rulesGenerated):
    rules = []
    for rule in rulesGenerated[len(rulesGenerated)-2]:
        rules.append(rule)
...
def start():
    ...
    if dataSetSelection == 1:
        ...
        fileName = "1000-out2_preprocessed_NP++.txt"
        dataPath = "1000receipts data set/"
    elif dataSetSelection == 5:
        fileName = "sample.txt"
    ...
    method = int(input("Enter 1 for F(k-1)*f(k) and 2 for F(k-1)*f(k-1):1\n"))
    ...
    rulesGenerated = ruleGen(freqItemList)
    #print(len(rulesGenerated))
    top10rules = top10RuleGen(rulesGenerated)
