...
def k_nearest_neighbors(data, predict, k=3):
    ...
    for group in data:
        for features in data[group]:
            # slow, works only in 2 dimensions, we can overcome that with single for looop
            # euclidean_distance = sqrt( (features[0]-predict[0])**2 + (features[1]-predict[1])**2 )
            # using numpy.sqrt and numpy.sum, also slow but faster than previous, no need addtional for loop
